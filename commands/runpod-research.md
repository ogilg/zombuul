...
2. **Ask the user two questions** using AskUserQuestion:
   ...
   - **Mode**: Which research mode to use. Options: (a) "Single experiment (/research-loop)" — runs one experiment and stops; (b) "Chained experiments (/research-ralph)" — runs experiments in a loop, each building on the last, until the research goal is met.
...
8. **Launch the research loop**: To avoid quoting issues, write a launch script to the pod, then run it. Use the command the user chose in step 2 (`/research-loop` or `/research-ralph`):
...
10. **Report to user**:
   ...
   - The pod will auto-terminate after the research loop finishes. Run `/runpod-stop` to terminate early if needed.
