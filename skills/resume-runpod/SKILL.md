...
1. **List pods**: Run `${CLAUDE_PLUGIN_ROOT}/scripts/runpod_ctl.py list`. Look for pods with EXITED status (these are paused).

2. If there are multiple paused pods, **ask the user** which one to resume using AskUserQuestion.

3. **Resume**: Run `${CLAUDE_PLUGIN_ROOT}/scripts/runpod_ctl.py resume <pod_id>`.
